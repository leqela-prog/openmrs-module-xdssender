...
public class CdaDataUtil {
	...
	public Organization createOrganization(Location location) {
		...
		Location parentLocation = location.getParentLocation() != null? location.getParentLocation(): location;
		
		// Identifiers
		retVal.setId(new SET<II>());
		LocationAttribute externalId = metadataUtil.getLocationAttribute(parentLocation,
		    XdsSenderConstants.ATTRIBUTE_NAME_EXTERNAL_ID);
		if (externalId != null)
			retVal.getId().add(parseIIFromString(externalId.getValue().toString()));
		retVal.getId().add(new II(config.getLocationRoot(), getLocationSiteCode(parentLocation)));   // USING LOCATION NAME, SO AS TO DEFAULT TO THE SERVICE POINT NAME
		
		// Name , etc. ?
		if (parentLocation.getName() != null) {
			retVal.setName(SET.createSET(new ON((EntityNameUse) null,
					Collections.singletonList(new ENXP(parentLocation.getName())))));  // USING THE PARENT LOCATION, SO AS TO DEFAULT TO FACILITY NAME
		} else if (parentLocation.getName() == null){
			retVal.setName(SET.createSET(new ON((EntityNameUse) null,
	...
	public CustodianOrganization getCustodianOrganization(Location location) {
		...
		Location parentLocation = shrLocation.getParentLocation() != null? shrLocation.getParentLocation(): shrLocation;
		...
		if (shrLocation == null) {
			...
			retVal.setName(new ON());
			retVal.getName().getParts().add(new ENXP(parentLocation.getName()));
			...
			if (idAttribute != null) {
				retVal.setId(SET.createSET(parseIIFromString(idAttribute.getValue().toString()),
						new II(config.getLocationRoot(), getLocationSiteCode(parentLocation))));
			} else {
				retVal.setId(SET.createSET(new II(config.getLocationRoot(),
						getLocationSiteCode(parentLocation))));
			}
			
			retVal.setAddr(createAddressSet(parentLocation));
			// TODO
			// retVal.setTelecom(new TEL());
			// retVal.getTelecom().setNullFlavor(NullFlavor.NoInformation);
		}
