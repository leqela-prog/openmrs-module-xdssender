 ...
 * @author JustinFyfe
 */
public class MedicalDocumentBuilderImpl implements DocumentBuilder {
	...
	public MedicalDocumentBuilderImpl() {
	}

	public MedicalDocumentBuilderImpl(Patient recordTarget, Encounter encounter, CdaDataUtil cdaDataUtil) {
	...
	public ClinicalDocument generate(Location location, Section... sections) {
		try {
			...
			CE<String> codeCE = new CE<String>(this.getTypeCode(), this.getCodeSystem(), "Medical Summary Note", "");
			retVal.setCode(codeCE);
			...
			retVal.setTemplateId(LIST.createLIST(new II(XdsSenderConstants.DOC_TEMPLATE_IMMUNIZATION_CONTENT)));
